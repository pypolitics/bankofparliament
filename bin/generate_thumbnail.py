...
def write_thumbnail(html_file, total_expenses, total_expenses_f, family_pretty, member_id, name, party, party_string, constituency, eu_ref_stance, salary_f, private_income_f, rental_income_f, total_income_f, gifts_f, gifts_outside_uk_f, direct_donations_f, indirect_donations_f, visits_outside_uk_f, total_freebies_f, shareholdings_percent, shareholdings_percent_items, shareholding_wealth_f, active_appointments, property_wealth_f, total_wealth_f,salary, private_income, rental_income, total_income, gifts, gifts_outside_uk, property_wealth, total_wealth, direct_donations, indirect_donations, visits_outside_uk, total_freebies, shareholdings, previous_appointments, family, miscellaneous, miscellaneous_f, miscellaneous_items, family_items, family_lobbyists_items, companies_house, twitter, gender):
    ...
    html += '\t\t\t\t\t</br>%s</br>\n' % total_expenses_f
    html += '\t\t\t\t</h2>\n'
    html += '\t\t\t\t<img src="lib/data/images/%s" class="img-responsive %s data-memberid=%s" style="width:100' % (photo, party_string.lower(), member_id) + r'%' + '; border-radius:50' + r'%' + '" alt="Image">\n'
    html += '\t\t\t</a>\n'
