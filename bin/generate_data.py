...
class MemberOfParliament():
	...
	def getMPIntrests(self):
		...
		headings = {}
		headings['1. Employment and earnings'] = Employment() # 1
		headings['2. (a) Support linked to an MP but received by a local party organisation or indirectly via a central party organisation'] = IndirectDonations() # 2 (a)
		headings['2. (b) Any other support not included in Category 2(a)'] = DirectDonations() # 2 (b)
		headings['3. Gifts, benefits and hospitality from UK sources'] = Gifts() # 3
		headings['4. Visits outside the UK'] = VisitsOutsideUK() # 4
		headings['5. Gifts and benefits from sources outside the UK'] = GiftsOutsideUK() # 5
		headings[u'6. Land and property portfolio: (i) value over \xa3100,000 and/or (ii) giving rental income of over \xa310,000 a year'] = Property() # 6
		headings[u"7. (i) Shareholdings: over 15% of issued share capital"] = Shareholdings(extended=self.extended, keywords=self.keywords, month=self.month, year=self.year, first=self.first, middle=self.middle, last=self.last, display=self.display) #7 (i)
		headings[u"7. (ii) Other shareholdings, valued at more than \xa370,000"] = OtherShareholdings(extended=self.extended, keywords=self.keywords, month=self.month, year=self.year, first=self.first, middle=self.middle, last=self.last, display=self.display) #7 (ii)
		headings['8. Miscellaneous'] = Miscellaneous() # 8
		headings['9. Family members employed and paid from parliamentary expenses'] = Family() # 9
		headings['10. Family members engaged in lobbying the public sector on behalf of a third party or client'] = FamilyLobbyists() # 10
		...
		for lin in soup:
			if not str(type(lin)) == "<class 'bs4.element.NavigableString'>":
				if lin.attrs['class'] == ['regmemcategory']:
				...
				elif lin.attrs['class'] == ['regmemitem']:

					if str(type(lin.next)) == "<class 'bs4.element.Tag'>":

						lin = lin.next
						if 'strong' in str(lin):
							lin = lin.next
					...
					br = lin.find_all('br')
					d = {'raw_string' : lin.text, }
					d[spl_name[0]] = spl_name[-1]
		...
		for k in y.keys():
			for i in y[k]:
				if headings.has_key(k):
					headings[k].add_entry(i['raw_string'], i)
		...
		self.employment = headings['1. Employment and earnings']
		self.indirect = headings['2. (a) Support linked to an MP but received by a local party organisation or indirectly via a central party organisation']
		self.direct = headings['2. (b) Any other support not included in Category 2(a)']
		self.gifts = headings['3. Gifts, benefits and hospitality from UK sources']
		self.visits_outside_uk = headings['4. Visits outside the UK']
		self.gifts_outside_uk = headings['5. Gifts and benefits from sources outside the UK']
		self.property = headings[u'6. Land and property portfolio: (i) value over \xa3100,000 and/or (ii) giving rental income of over \xa310,000 a year']
		self.shareholdings = headings[u"7. (i) Shareholdings: over 15% of issued share capital"]
		self.other_shareholdings = headings[u"7. (ii) Other shareholdings, valued at more than \xa370,000"]
		self.miscellaneous = headings['8. Miscellaneous']
		self.family = headings['9. Family members employed and paid from parliamentary expenses']
		self.family_lobbyists = headings['10. Family members engaged in lobbying the public sector on behalf of a third party or client']
